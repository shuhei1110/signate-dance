...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.status == 'train':
            video_path = data_config.train_dir + row.loc['file']
        elif self.status == 'valid':
            video_path = data_config.train_dir + row.loc['file']
        elif self.status == 'test':
            video_path = data_config.test_dir + row.loc['file']
        else:
...
class ClassificationModel(nn.Module):
    def __init__(self):
       ...
       self.encoder = timm.create_model(model_config.encoder, pretrained=True, in_chans=model_config.ch_size, num_classes=512)
       self.gru = nn.GRU(512, 128, batch_first=True, bidirectional=False, num_layers=1, dropout=0)
...
model = ClassificationModel().to(base_config.device)
...
for epoch in range(model_config.epochs):
    ...
    for batch_idx, (images, labels) in enumerate(tqdm(train_dataloader)):
        images = images.to(base_config.device).float()
        labels = labels.type(torch.LongTensor).to(base_config.device)
